...
if ($session->param ('loggedin'))
{
   ...
   my $cookie = cookie (-name=>'perl260',
    ...
    my $sql = qq{UPDATE tblclasses SET classname="$classname", department="$department", classnum="$classnum", grade="$grade", credits=$credits  WHERE classid = $classid};
    my $sth = $dbh->prepare($sql);
    my $recordCount = $sth->execute();
    
    
    print header(), start_html(-title=>"Edit Class | UND Course Manager");
    #print redirect (-cookie=>$cookie, -location=>'/cgi-bin/index.pl'), start_html(), end_html();
    print br, "<h1 align=center>UND Course Manager</h1>","<p align=center><span style=\"font-size:large;\">Update successful.</span></p>", br;
